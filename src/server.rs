...
async fn handle_connection(mut socket: TcpStream, storage: Arc<Storage>) -> anyhow::Result<()> {
    ...
    loop {
        ...
        if length > MAX_FRAME_LENGTH {
            let resp = protocol::error(version, stream, 0x000A, "Frame too large", state.compression);
            socket.write_all(&resp).await?;
            return Ok(());
        }
        ...
        let resp = handle_request(&storage, &mut state, opcode, &body, stream)
            .unwrap_or_else(|e| protocol::error(version, stream, e.code, &e.message, state.compression));
        socket.write_all(&resp).await?;
    }
}
...
fn handle_request(
    ...
    match opcode {
        ...
        RequestOpcode::Startup => {
            let mut buf: &[u8] = body;
            let startup = Startup::deserialize_with_features(&mut buf, &features).map_err(|_| {
                query::QueryError {
                    code: 0x000A,
        ...
        RequestOpcode::Query => {
            ...
            match query::execute(storage, &mut state.keyspace, &query_text) {
                Ok(query::Response::Rows { keyspace, table, cols, rows }) => Ok(protocol::result_rows(
                    version,
                    stream,
                    &keyspace,
                    &table,
                ...
                Ok(query::Response::Void) => Ok(protocol::result_void(version, stream, state.compression)),
                Ok(query::Response::SetKeyspace(ks)) => {
                    Ok(protocol::result_set_keyspace(version, stream, &ks, state.compression))
                }
                Ok(query::Response::SchemaChange { change, target, keyspace, table }) => Ok(
                    protocol::result_schema_change(version, stream, change, target, &keyspace, &table, state.compression),
                ),
                Err(e) => Err(e),
            }
        }
        RequestOpcode::Prepare | RequestOpcode::Execute | RequestOpcode::Batch => Err(query::QueryError {
            code: 0x2200,
            message: "Prepared statements and batches are not supported yet".to_string(),
        }),
